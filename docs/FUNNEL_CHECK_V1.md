...
|---|---|---|---|
| `x` | プロフィール URL | ここに貼った追跡リンクが押された回数だけ | X のページ取得と投稿の巡回（規約）。プロフィールを見た人数 |
| `line` | 友だち追加 URL（lin.ee / line.me/R/ti/p / liff） | URL が生きているか、飛び先が LINE の友だち追加ページか | リッチメニュー・配信の中身（権限が要る）。友だち追加の数 |
| `web` | 任意の URL | 状態コード、最終 URL、タイトル、**次の段へのリンク（この段の追跡リンクか、次の段の URL そのもの）が本文にあるか**、スマホ幅の見た目（第2段階） | ログインの先、フォーム送信 |

外部 URL の取得は、転送を1段ずつ形で確かめて追い、名前解決の結果が社内・自分自身・リンクローカルのアドレスなら接続しない（`fetch-safe.ts`）。本文は読み込みながら上限（512KB）で止める。「開けない」は「URL が誤っている」とは限らない（相手がボットを弾いている・重い）ので、理由の文でその区別を書く。
...
## 4. 追跡リンク（2026-09-20 に Codex レビュー F02・F03・F06 で改めた）

- 形: `https://mado.shikumiai.com/go/<code>`。`code` は英数字8文字（読み違えやすい文字を除く）
- **段 i のリンクは「段 i に貼るもの」で、押すと段 i+1 へ飛ぶ。** X のプロフィールに貼るリンクは LINE へ送り、LINE のメニューに貼るリンクはサイトへ送る。最後の段には貼るリンクが無い
- 記録するもの: 導線 ID、段の番号、時刻、User-Agent のハッシュ、Referer のホスト名だけ。IP は保存しない
- **数えるのは「押された回数」だけ。** 画面にもそう書く。UA のハッシュは人を区別できない（別人でも同じ値になる）ので、人数・離脱率・成約率は出さない。LINE の友だち追加や予約の完了も、その事象を記録する仕組みを作るまでは「測っていない」と書く
- 明らかなボット（UA に bot/crawler/preview を含む）は数えない
- 導線を消しても行は残す（`archived_at` を立てる）。貼ってしまったリンクは飛び先を失わない
...
- 追跡リンクを押すと次の段に着き、画面に押された回数が出る（人数とは書かない）
