...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/site-content?orderId=${orderId}&email=${encodeURIComponent(email)}`)
      ...
      .then((data) => {
        ...
        setTemplateId((config.templateId || "warm-craft").replace(/-(?:mid|pro)$/, ""));
        setPlan(normalizePlanId(data.plan || "otameshi"));
        setRepoName(data.repoName || "");

        // セクション初期化
        const initialSections = getSections(config);
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      const email = session?.user?.email;
      const changeArray: Array<Record<string, unknown>> = [];

      // テキスト/画像変更
      for (const [fieldId, c] of changes.entries()) {
        const isImage = c.newValue.startsWith("data:image/");
        if (isImage) {
          // 画像: GitHubリポに保存 + configの画像パスも更新
          const safeName = fieldId.replace(/\./g, "-").replace(/[^a-zA-Z0-9-]/g, "");
          const imagePath = `public/images/${safeName}.jpg`;
          changeArray.push({
            type: "image",
            imagePath,
            imageData: c.newValue.split(",")[1],
            configPath: fieldId,
            imageUrl: `/images/${safeName}.jpg`,
          });
        } else {
          changeArray.push({ type: "text", configPath: fieldId, newValue: c.newValue });
        }
      }

      // セクション変更
      if (sectionsChanged) {
        changeArray.push({ type: "sections", sections });
      }
      ...
      if (!res.ok && !data.applied) {
        ...
        setApplyResult({
          applied: 0,
          failed: data.failed || [{ field: "全体", error: data.error || "不明なエラー" }],
          message: data.error || "反映に失敗しました",
        });
      } else {
        // 成功（一部失敗含む）
        setApplyResult({
  ...
  }, [changes, orderId, session, sectionsChanged, sections]);
